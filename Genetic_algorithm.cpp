...
int main() // функция номер 20
{
    ...
    int q;
    /*cout << "Будете вводить переменные(1-да): " << endl;
    cin >> q;*/
    q = 0;
    switch (q)
    {
    case 1:
        cout << "Введите вероятность скрещивания (рекомбинации) [0 - 100]: " << endl;
        cin >> probability_c;
        if (probability_c < 0 || probability_c > 100) {
            cout << "Нкорректные данные" << endl;
            return 0;
        }
        cout << "Введите вероятность мутации [0 - 100]: " << endl;
        cin >> probability_m;
        if (probability_m < 0 || probability_m > 100) {
            cout << "Нкорректные данные" << endl;
            return 0;
        }
        cout << "Введите максимальное количество итераций [>0]: " << endl;
        cin >> iter_max;
        if (iter_max < 0) {
            cout << "Нкорректные данные" << endl;
            return 0;
    ...
    do
    {
        ...
        crossing(population_max, k_individuals_max, k_parence, probability_c, LEN);   // Нахождение и скрещивание следующего поколения
